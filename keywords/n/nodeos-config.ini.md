...
노드의 환경설정 파일이다. [nodeos](nodeos.md)를 구동하면 자동 생성된다. 한번도 구동하지 않았다면 파일이 존재하지 않기 때문에 오류 여부와 관계없이 일단 한번이라도 실행 시켜야 한다. 
...
아래에 표시된 기능들은 v1.5.3 기준으로 작성 되었다. 버전에 따라서 환경설정 내용이 조금 다르거나 없을 수 있다.
...
* `bnet-endpoint` :
* `bnet-follow-irreversible` : 
* `bnet-threads` :
* `bnet-connect` :
* `bnet-no-trx` :
* bnet-peer-log-format : 
* blocks-dir : 
* checkpoint : 
* wasm-runtime : 
...
* max-cleanup-time-msec
* network-version-match
* sync-fetch-span
* max-implicit-request
* use-socket-read-watermark
* peer-log-format
